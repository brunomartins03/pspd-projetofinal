...
void UmaVida(int* tabulIn, int* tabulOut, int tam, int loc_lines) {
  ...
  for (int i = 1; i <= loc_lines; i++) {
    for (int j = 1; j <= tam; j++) {
      ...
      if (tabulIn[ind2d(i,j,tam)] && vizviv < 2) tabulOut[ind2d(i,j,tam)] = 0;
      else if (tabulIn[ind2d(i,j,tam)] && vizviv > 3) tabulOut[ind2d(i,j,tam)] = 0;
      else if (!tabulIn[ind2d(i,j,tam)] && vizviv == 3) tabulOut[ind2d(i,j,tam)] = 1;
      else tabulOut[ind2d(i,j,tam)] = tabulIn[ind2d(i,j,tam)];
    }
  }
}
